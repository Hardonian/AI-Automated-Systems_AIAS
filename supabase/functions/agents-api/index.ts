    ...
    switch (method) {
      case 'GET':
        if (agentId && action === 'interactions') {
          ...
          return await getAgentInteractions(
            supabaseClient,
            agentId,
            tenantId,
            user.id
      ...
      case 'POST':
        if (agentId && action === 'interact') {
          ...
          return await interactWithAgent(
            supabaseClient,
            agentId,
            body,
            tenantId,
...
async function interactWithAgent(
  ...
  try {
    ...
    const outputText = await processAgentInteraction(
      agent,
      body.inputText,
      body.context || {}
    );
...
async function getAgentInteractions(
  supabaseClient: any,
  agentId: string,
  tenantId: string,
  userId: string
...
async function trainAgent(
  ...
  return new Response(JSON.stringify({ message: 'Agent training started' }), {
    headers: { ...corsHeaders, 'Content-Type': 'application/json' },
  });
}
...
async function processAgentInteraction(
  ...
  if (tone === 'professional') {
    response +=
      'I understand your request and will provide a professional response. ';
  } else if (tone === 'casual') {
  ...
  response +=
    "here's what I can tell you about this topic. This is a simulated response from the AI agent.";
...
async function updateAgentMetrics(
  ...
  const newSuccessCount = success
    ? (metrics.successRate * metrics.totalInteractions) / 100 + 1
    : (metrics.successRate * metrics.totalInteractions) / 100;
  const newSuccessRate = (newSuccessCount / newTotalInteractions) * 100;
  const newAverageResponseTime =
    (metrics.averageResponseTime * metrics.totalInteractions + processingTime) /

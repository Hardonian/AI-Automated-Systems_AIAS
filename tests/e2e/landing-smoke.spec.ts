...
test.describe('@smoke AIAS Landing & Workflow Smoke Test', () => {
  test('Home page loads with key sections', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /Agentic Automation Consultancy/i })).toBeVisible();

    const cta = page.getByRole('link', { name: /Book (a )?Strategy Call|Book Diagnostic/i }).first();
  ...
  test('Contact intake flow is functional', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /Start with a focused automation conversation/i })).toBeVisible();
    await expect(page.getByRole('heading', { name: /Organization and problem context/i })).toBeVisible();
  ...
  test('Route-first nav and footer integrity', async ({ page }) => {
    ...
    await expect(page.getByRole('link', { name: /Navigate to Process|Navigate to What AIAS Does/i }).first()).toBeVisible();

...
export const PARTNERSHIP_TIERS: PartnershipTier[] = [
  ...
  },
  {
    ...
    name: 'Gold Partner',
    description:
      'Premium partnership for large agencies and system integrators',
    requirements: {
      minRevenue: 50000,
      minCustomers: 20,
      minReferrals: 10,
  ...
  },
  {
    ...
    description:
      'Elite partnership for strategic partners and enterprise resellers',
    requirements: {
      minRevenue: 100000,
      minCustomers: 50,
      minReferrals: 25,
...
export class PartnershipService {
  ...
  async scoreLead(lead: Lead): Promise<{ score: number; factors: string[] }> {
    ...
    if (
      executiveTitles.some(title => lead.title?.toLowerCase().includes(title))
    ) {
      score += 25;
  ...
  async createWhiteLabelConfig(
    partnerId: string,
    config: Partner['whiteLabelConfig']
  ): Promise<void> {
  ...
  async getPartnerPerformance(
    ...
    const qualifiedReferrals =
      referrals?.filter((r: { status: string }) => r.status === 'qualified')
        .length || 0;
    const paidReferrals =
      referrals?.filter((r: { status: string }) => r.status === 'paid')
        .length || 0;
    const totalCommission =
      referrals?.reduce(
        (sum: number, r: { commission: number }) => sum + r.commission,
        0
      ) || 0;
    const totalLeadValue =
      leads?.reduce((sum: number, l: { value: number }) => sum + l.value, 0) ||
      0;
    ...
    return {
      ...
      totalLeadValue,
      conversionRate:
        totalReferrals > 0 ? (qualifiedReferrals / totalReferrals) * 100 : 0,
      averageCommission:
        totalReferrals > 0 ? totalCommission / totalReferrals : 0,
    };
  }
...
export const generateLeadMagnet = (
  type: 'ebook' | 'webinar' | 'template' | 'demo'
) => {
  const magnets = {
    ebook: {
      title: 'The Complete Guide to AI Automation for Business',
      description:
        'Learn how to implement AI automation in your business with our comprehensive guide.',
    ...
    webinar: {
      ...
      description:
        'Join our experts for a live session on AI automation best practices.',
      value:
        'Interactive content that allows for direct engagement and qualification',
    },
    template: {
    ...
    demo: {
      ...
      description:
        'See how AI automation can transform your specific business processes.',
      value:
        'High-intent lead generation with immediate qualification opportunity',
    },
  };

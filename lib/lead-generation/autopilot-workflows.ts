...
class AutopilotWorkflowService {
    ...
    try {
      ...
      for (const workflow of workflows) {
        try {
          ...
          if (
            workflow.conditions &&
            !this.checkConditions(context, workflow.conditions)
          ) {
            continue;
          }
  ...
  private async executeSendEmail(
    ...
    if (!templateId || (!leadId && !email)) {
      throw new Error(
        'Template ID and Lead ID or email required for send_email action'
      );
    }
    ...
    const variables: Record<string, string> = {
      ...
      automationName:
        lead?.automation_name || (context.automationName as string) || '',
  ...
  private async executeSyncCRM(
    ...
    if (!leadId || !provider || !apiKey) {
      throw new Error(
        'Lead ID, provider, and API key required for sync_crm action'
      );
    }
    ...
    await crmIntegrationService.syncLeadToCRM(
      leadId,
      {
        provider: provider as 'salesforce' | 'hubspot' | 'pipedrive' | 'custom',
        apiKey,
        apiUrl,
      },
      tenantId
  ...
  private async executeStartNurturing(
    ...
    if (!leadId || !sequenceId) {
      throw new Error(
        'Lead ID and sequence ID required for start_nurturing action'
      );
    }
  ...
  async createWorkflow(
    workflow: AutopilotWorkflowInput,
    tenantId?: string
  ): Promise<string> {

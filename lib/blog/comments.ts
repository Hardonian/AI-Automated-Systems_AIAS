...
export function moderateComment(comment: Comment): ModerationResult {
  ...
  // Check for spam
  if (
    moderationRules.spam.keywords.some(keyword => content.includes(keyword))
  ) {
    score -= 50;
  ...
  // Check for toxicity
  const toxicityMatches = moderationRules.toxicity.keywords.filter(keyword =>
  ...
  const systemsThinkingMatches =
    moderationRules.systemsThinking.encourage.filter(keyword =>
      content.includes(keyword)
    );
...
export function generateSystemsThinkingInsight(
  comment: Comment,
  _article: BlogArticle | Partial<BlogArticle>
): string {
  const content = comment.content.toLowerCase();

  // Check for systems thinking concepts
  if (
    content.includes('systems thinking') ||
    content.includes('multi-perspective')
  ) {

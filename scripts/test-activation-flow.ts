...
async function testActivationFlow() {
  ...
  try {
    ...
    const integrationResponse = await fetch(
      'http://localhost:3000/api/integrations/shopify',
      {
        method: 'POST',
        headers: {
    ...
    if (!integrationResponse.ok) {
      console.log(
        '⚠️ Integration connection test skipped (requires auth token)'
      );
    } else {
    ...
    if (activationEvent) {
      ...
      console.log(
        '⚠️ user_activated event not found (user may not have activated yet)'
      );
    }

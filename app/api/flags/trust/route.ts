...
export async function GET() {
  try {
    ...
    if (isEdgeConfigAvailable()) {
      const trustFlags = {
        trust_audit_enabled:
          (await getEdgeConfigValue<boolean>('feature:trust_audit_enabled')) ??
          true,
        trust_ledger_enabled:
          (await getEdgeConfigValue<boolean>('feature:trust_ledger_enabled')) ??
          true,
        trust_scoring_enabled:
          (await getEdgeConfigValue<boolean>(
            'feature:trust_scoring_enabled'
          )) ?? false,
        trust_badges_enabled:
          (await getEdgeConfigValue<boolean>('feature:trust_badges_enabled')) ??
          true,

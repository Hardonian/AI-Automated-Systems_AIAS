...
export default function SuggestionsDrawer({ userId }: { userId: string }) {
  ...
  return (
    <>
      <button aria-controls="agent-drawer" aria-expanded={open} className="fixed bottom-4 right-4 rounded-full bg-accent text-accent-fg h-12 w-12 shadow-card" onClick={() => setOpen(v => !v)}>★</button>
      {open && (
        <aside className="fixed bottom-20 right-4 w-[92vw] sm:w-[420px] max-h-[70vh] overflow-auto rounded-2xl border border-border bg-card p-4 shadow-card" id="agent-drawer">

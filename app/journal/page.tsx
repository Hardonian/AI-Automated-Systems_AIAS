...
export default function JournalPage(){
  ...
  async function save() {
    ...
    if (!user) {return;}
    const { data: _data, error } = await supabase.from("journal_entries").insert({ user_id: user.id, body: text } as Record<string, unknown>).select("*").single();
    setSaved(!error);
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <textarea className="w-full rounded-xl border border-border p-3" placeholder="Reflect on today's progress…" rows={8}

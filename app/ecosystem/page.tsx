      ...
      <PageSection>
        <SurfaceCard>
          <h2 className="text-2xl font-bold">
            {siteContent.ecosystemPage.diagram.title}
          </h2>
          <svg
            aria-label="Ecosystem architecture flow diagram"
            ...
            {siteContent.ecosystemPage.diagram.nodes.map((node, index) => (
              <g key={node} transform={`translate(${20 + index * 175},40)`}>
                <rect
                  ...
                  width="150"
                />
                <text fontSize="16" textAnchor="middle" x="75" y="52">
                  {node}
                </text>
                {index < 4 ? (
                  <text fontSize="28" x="160" y="56">
                    →
                  </text>
                ) : null}
              </g>
      ...
      <PageSection>
        <div className="grid gap-6 md:grid-cols-2">
          ...
          <SurfaceCard>
            <h2 className="text-2xl font-bold">
              {
                siteContent.ecosystemPage.narrative.determinismVsIntelligence
                  .title
              }
            </h2>
          ...
          <SurfaceCard>
            <h2 className="text-2xl font-bold">
              {siteContent.ecosystemPage.narrative.deploymentModels.title}
            </h2>
            <ul className="mt-4 space-y-2 text-muted-foreground">
              {siteContent.ecosystemPage.narrative.deploymentModels.items.map(
                (item) => (
                  <li key={item}>• {item}</li>
                ),
              )}
          ...
          <SurfaceCard>
            <h2 className="text-2xl font-bold">
              {siteContent.ecosystemPage.narrative.governancePrinciples.title}
            </h2>
            <ul className="mt-4 space-y-2 text-muted-foreground">
              {siteContent.ecosystemPage.narrative.governancePrinciples.items.map(
                (item) => (
                  <li key={item}>• {item}</li>
                ),
              )}

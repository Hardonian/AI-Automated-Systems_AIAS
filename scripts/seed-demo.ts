  ...
  try {
    ...
    for (const tenantData of demoTenants) {
      ...
      if (error && !error.message.includes('duplicate')) {
        console.error(
          `  ✗ Failed to create tenant ${tenantData.name}:`,
          error.message
        );
      } else {
        console.log(`  ✓ Created tenant: ${tenantData.name}`);
        if (tenant) {
          demoData.tenants.push(tenant);
        }
      }
    }
    ...
    const sampleAgents = [
      ...
      },
      {
        ...
        model: 'gpt-4',
        instructions:
          'You are a content writer. Create engaging, SEO-friendly content.',
        enabled: true,
      },
    ];
    ...
    for (const workflowData of sampleWorkflows) {
      ...
      if (error) {
        console.error(
          `  ✗ Failed to create workflow ${workflowData.name}:`,
          error.message
        );
      } else {

...
const envSchema = z
  .object({
    ...
    // Prisma
    PRISMA_CLIENT_ENGINE_TYPE: z
      .enum(['library', 'wasm', 'binary'])
      .optional()
      .default('library'),
  })
...
function getEnv() {
  ...
  return (parsed.success ? parsed.data : process.env) as z.infer<
    typeof envSchema
  >;
}
...
export function getValidatedEnvVar(varName: string): string | undefined {
  ...
  if (result.success && result.data) {
    return result.data[varName as keyof typeof result.data] as
      | string
      | undefined;
  }

...
export async function GET(): Promise<NextResponse<HealthCheckResult>> {
  ...
  const [dbResult, restResult, authResult, rlsResult, storageResult] =
    await Promise.allSettled([
      ...
      (async () => {
        ...
        if (profilesError) {
          ...
          const { error: workflowsError } = await supabaseService
            .from('workflows')
            .select('id')
            .limit(1);
          if (workflowsError) {
            ...
            const { error: tenantsError } = await supabaseService
              .from('tenants')
              .select('id')
              .limit(1);
            if (tenantsError) {
        ...
        return {
          ...
          ...(triedTables.length > 0 && {
            note: `Checked tables: ${triedTables.join(', ')}`,
          }),
        };
      })(),
  ...
  if (rlsResult.status === 'fulfilled') {
    ...
    checks.rls = {
      ok: false,
      error:
        rlsResult.reason instanceof Error
          ? rlsResult.reason.message
          : String(rlsResult.reason),
    };
    checks.ok = false;
  }

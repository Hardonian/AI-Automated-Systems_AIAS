...
function ArchitectureDiagram({ nodes }: { nodes: string[] }) {
  return (
    <svg aria-label='Blueprint architecture diagram' className='h-auto w-full rounded-md border bg-background p-4' viewBox='0 0 640 220'>
      {nodes.map((node, index) => {
        const x = 30 + index * 150;
        return (
          <g key={node}>
            <rect fill='currentColor' fillOpacity='0.06' height='70' rx='10' width='130' x={x} y={75} />
            <text fontSize='13' textAnchor='middle' x={x + 65} y={110}>{node}</text>
            {index < nodes.length - 1 ? <line stroke='currentColor' strokeOpacity='0.5' x1={x + 130} x2={x + 150} y1={110} y2={110} /> : null}
          </g>
        );
      })}
    </svg>
        ...
        <div className='grid gap-6 md:grid-cols-2'>
          ...
          <SurfaceCard>
            ...
            <ul className='mt-3 space-y-2 text-muted-foreground'>
              {blueprint.constraints.map(constraint => <li key={constraint}>• {constraint}</li>)}
            </ul>
          </SurfaceCard>
        </div>
      </PageSection>
      ...
      <PageSection width='narrow'>
        <SurfaceCard>
          ...
          <ul className='mt-4 space-y-2 text-muted-foreground'>
            {blueprint.implementationNotes.map(note => <li key={note}>• {note}</li>)}
          </ul>
          <a className='mt-5 inline-flex text-sm font-semibold text-primary underline underline-offset-4' href={blueprint.downloadPath}>Download artifact</a>
        </SurfaceCard>
      </PageSection>
    </>
  );

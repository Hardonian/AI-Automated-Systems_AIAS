...
describe('caseStudies generator', () => {
  ...
  it('should generate case studies directly from project configs', () => {
    ...
    expect(reach?.implementationHighlights).toEqual(['GTM strategy', 'Lead gen']);
  ...
  it('should map legacy case studies and merge arrays based on architecture mentions', () => {
    ...
    expect(mockCase?.architecture).toEqual(['Reach and Zeo architecture', 'Constraint 1']);
    ...
    expect(mockCase?.automationWins).toEqual(['Next 1', 'Lower CAC', 'Reduced overhead']);
  ...
  it('should assume all projects are involved if no explicit mentions are found', () => {
    const anotherCase = caseStudies.find(c => c.slug === 'another-legacy-case');
    ...
    expect(anotherCase?.automationWins).toEqual(['Lower CAC', 'Reduced overhead', 'Bank-grade security']);

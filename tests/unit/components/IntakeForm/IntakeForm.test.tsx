...
async function fillAndSubmitForm(user: ReturnType<typeof userEvent.setup>) {
  ...
  const getSelectTrigger = (name: RegExp) =>
  ...
  await selectOption(
    /problem category/i,
    "Manual operations and repeated tasks",
  );
  ...
  await selectOption(
    /failure mode/i,
    "Hallucination and output quality issues",
  );
  ...
  await user.click(
    screen.getByLabelText("One-off diagnostic or implementation"),
  );

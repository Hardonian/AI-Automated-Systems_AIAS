 ...
 * Validation function
 */
function validateForm(context: DemoFormContext): {
  valid: boolean;
    ...
    states: {
      ...
      submitting: {
        ...
        invoke: {
          id: 'submitForm',
          src: fromPromise(
            async ({ input }: { input: DemoFormContext['formData'] }) => {
              return await retry(() => submitForm(input), 3, 1000);
            }
          ),
          input: ({ context }: { context: DemoFormContext }) =>
            context.formData,
          onDone: {
            target: 'success',
            actions: assign({

...
interface CacheService {
  get(
    key: string,
    options?: { ttl?: number; tenantId?: string; tags?: string[] }
  ): Promise<unknown>;
  set(
    key: string,
    value: unknown,
      ...
      try {
          ...
          if (!resourceCheck.allowed) {
            ...
            return NextResponse.json(
              { error: formatted.message },
              {
                ...
                headers: {
                  'Retry-After': String(
                    Math.ceil((resourceCheck.resetAt - Date.now()) / 1000)
                  ),
                },
              }
            );
        ...
        if (normalizedOptions.maxBodySize) {
          ...
          if (!checkRequestSize(body, normalizedOptions.maxBodySize)) {
            const error = new ValidationError('Request too large', undefined, {
              maxSize: normalizedOptions.maxBodySize,
            });
            const formatted = formatError(error);
            return NextResponse.json(
              { error: formatted.message },
        ...
        // Get user ID from request with proper JWT verification
        let userId: string | null = null;
        if (normalizedOptions.requireAuth || normalizedOptions.requireTenant) {
          const authHeader = request.headers.get('authorization');
          if (authHeader?.startsWith('Bearer ')) {
            // Extract and verify JWT token using jose library
            try {
              const token = authHeader.substring(7);
              const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
              if (!supabaseUrl) {
                throw new AuthenticationError('Auth configuration error');
              }
              // Verify token against Supabase JWKS endpoint
        ...
        if (normalizedOptions.validateBody) {
          try {
            ...
            const validation = validateInput(
              normalizedOptions.validateBody,
              body
            );

            if (!validation.success) {
              const errorDetails =
                isObject(validation.error) &&
                'issues' in validation.error &&
        ...
        try {
          const { telemetry } =
            await import('@/lib/monitoring/enhanced-telemetry');

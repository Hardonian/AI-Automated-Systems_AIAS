...
export function PreTestQuestionnaire({
  onComplete,
  onSkip,
  canDismiss = false,
}: PreTestQuestionnaireProps) {
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      logger.error(
        'Failed to save pretest answers',
  ...
  return (
    <Card className='mx-auto max-w-2xl border-2 border-primary/30 shadow-xl'>
      {canDismiss && (
            ...
            <div className='space-y-3'>
              {GOALS.map(goal => (
                <div key={goal.value} className='flex items-center space-x-2'>
                  ...
                  <Label className='flex-1 cursor-pointer' htmlFor={goal.value}>
                    {goal.label}
                  </Label>
                </div>
              ))}
          ...
          <div className='space-y-6'>
            <div>
              ...
              <RadioGroup
                ...
                onValueChange={value =>
                  setAnswers({ ...answers, listSize: value })
                }
              >
            ...
            <div>
              ...
              <RadioGroup
                ...
                onValueChange={value =>
                  setAnswers({ ...answers, emailFrequency: value })
                }
              >
                <div className='space-y-2'>
              ...
              <div className='space-y-2'>
                {CONTENT_TYPES.map(type => (
                  <div key={type.value} className='flex items-center space-x-2'>
                    <Checkbox
                      checked={(answers.contentTypes || []).includes(
                        type.value
                      )}
                      id={type.value}
                      onCheckedChange={checked => {
                        ...
                        if (checked) {
                          setAnswers({
                            ...answers,
                            contentTypes: [...current, type.value],
                          });
                        } else {
                          setAnswers({
                            ...answers,
                            contentTypes: current.filter(t => t !== type.value),
                          });
                        }
                      }}
                    />
                ...
                <Checkbox
                  ...
                  onCheckedChange={checked => {
                    ...
                    if (checked) {
                      setAnswers({
                        ...answers,
                        painPoints: [...current, point.value],
                      });
                    } else {
                      setAnswers({
                        ...answers,
                        painPoints: current.filter(p => p !== point.value),
                      });
                    }
                  }}
                />
                <Label className='flex-1 cursor-pointer' htmlFor={point.value}>
                  {point.label}
                </Label>
              </div>
            ))}

...
import { CheckCircle2, Copy, Download, Sparkles, FileText, ListChecks, Code2, ArrowRight } from 'lucide-react';
...
export function WorkflowSandbox() {
  ...
  const [activeOutputTab, setActiveOutputTab] = useState<'markdown' | 'checklist' | 'json'>('markdown');
  ...
  const normalizedValues = useMemo(
    () => ({
      ...
      constraints: values.constraints || 'Deterministic accuracy required, zero unauthorized PII leakage',
  ...
  return (
    <section className="border-b-2 border-border bg-muted/20 px-4 py-24" id="workflow-sandbox">
                ...
                <form className="space-y-5" onSubmit={handleSubmit}>
                  {workflowSandbox.inputForm.fields.map((field: WorkflowField) => (
                    <div key={field.id} className="space-y-1.5">
                      <Label htmlFor={field.id} className="font-mono text-xs font-bold uppercase text-foreground">
                        {field.label}
                      </Label>
                      {field.type === 'select' ? (
                        <Select
                        ...
                        >
                          <SelectTrigger id={field.id} className="rounded-none border-2 border-border font-mono text-xs">
          ...
          <div className="lg:col-span-6">
            <SurfaceCard className="p-6 sm:p-8 border-2 border-primary bg-card h-full flex flex-col justify-between shadow-card">
              ...
              <div className="mt-6 border-t-2 border-border pt-4 space-y-3">
                ...
                {copyStatus && (
                  <p className="text-xs font-mono font-bold text-center text-primary">{copyStatus}</p>
                )}

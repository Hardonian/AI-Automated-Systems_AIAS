...
async function executeWorkflowAsync(
  ...
  try {
    ...
    if (workflowError || !workflow) {
      throw new SystemError(
        `Workflow not found or disabled: ${workflowError?.message}`
      );
    }
...
export async function POST(
  request: NextRequest,
  { params }: { params: Promise<{ tenant_id: string; secret: string }> }
): Promise<NextResponse> {
  try {
    const { tenant_id: tenantId, secret } = await params;
    ...
    const pathSecret = (secret || '').trim();
    const isPlaceholderSecret = pathSecret === '_' || pathSecret === 'header';
    const secretToUse = isPlaceholderSecret ? secretFromHeader : pathSecret;
    ...
    const verification = await verifyWebhookSecret(tenantId, secretToUse);
    ...
    executeWorkflowAsync(
      ...
      logger.error(
        'Failed to execute webhook workflow',
    ...
    const { tenant_id: tenantIdForError } = await params;

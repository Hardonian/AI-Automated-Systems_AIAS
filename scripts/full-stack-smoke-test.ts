        ...
        for (const line of lines) {
          ...
          if (trimmed && !trimmed.startsWith('#')) {
            ...
            if (match) {
              ...
              if (vitePrefix === 'VITE_') {
                if (key === 'SUPABASE_URL') {
                  mappedKey = 'NEXT_PUBLIC_SUPABASE_URL';
                } else if (
                  key === 'SUPABASE_ANON_KEY' ||
                  key === 'SUPABASE_PUBLISHABLE_KEY'
                ) {
                  mappedKey = 'NEXT_PUBLIC_SUPABASE_ANON_KEY';
                } else if (key === 'SUPABASE_PROJECT_ID') {
                  mappedKey = 'SUPABASE_PROJECT_REF';
                }
              }
              ...
              if (
                mappedKey &&
                value &&
                (EXPECTED_ENV_VARS.includes(mappedKey) ||
                  (key && EXPECTED_ENV_VARS.includes(key)))
              ) {
...
async function checkVercelEnv(): Promise<{
  ...
  if (!vercelToken) {
    testResults.push({
      ...
      passed: false,
      message:
        'VERCEL_TOKEN not set. Cannot pull Vercel environment variables.',
    });
    return result;
  }
  ...
  if (Object.keys(secrets).length === 0 && process.env.GITHUB_TOKEN) {
    try {
      ...
      testResults.push({
        ...
        passed: true,
        message:
          'GitHub secrets exist (values cannot be read via API for security)',
    ...
    try {
      ...
      try {
        ...
        results.push({
          ...
          details: {
            note: 'Install pg library to test actual database connectivity',
          },
        });
      }
    } catch (error: any) {
...
async function testVercelEndpoints(): Promise<TestResult[]> {
  ...
  const endpoints = ['/api/healthz', '/api/health', '/api/telemetry'];
...
async function testLocalDevelopment(): Promise<TestResult[]> {
  ...
  try {
    ...
    const majorVersion = versionStr
      ? parseInt(versionStr.split('.')[0] || '0')
      : 0;
  ...
  if (existsSync(schemaPath)) {
    ...
    if (process.env.DATABASE_URL) {
      ...
      if (commandExists('prisma') || commandExists('npx')) {
        try {
          const cmd = commandExists('prisma')
  ...
  if (!commandExists('prisma') && !commandExists('npx')) {
    results.push({
      ...
      passed: false,
      message:
        'Prisma CLI not found. Install with: npm install -g prisma or use npx',
    });
  }
...
function generateAutoFixSteps(matrix: EnvVarSource[]): string[] {
  ...
  for (const source of matrix) {
    ...
    if (!source.vercelProd) {
      missing.push('Vercel Production');
    }
    if (!source.vercelPreview) {
      missing.push('Vercel Preview');
    }
    if (!source.vercelDev) {
      missing.push('Vercel Development');
    }
    ...
    if (missing.length > 0) {
      ...
      if (
        source.name.startsWith('SUPABASE_') ||
        source.name.startsWith('NEXT_PUBLIC_SUPABASE_')
      ) {
        steps.push(
          `  → Get ${source.name} from Supabase Dashboard → Settings → API`
        );
    ...
    if (values.length > 1) {
      ...
      if (uniqueValues.size > 1) {
        steps.push(
          `⚠ Mismatch detected for ${source.name} across different sources`
        );
        steps.push(`  → Authoritative source: Supabase Dashboard`);
        steps.push(
          `  → Sync command: vercel env add ${source.name} production --token $VERCEL_TOKEN`
        );
        steps.push(
          `  → GitHub: gh secret set ${source.name} --body "$(get-value-from-supabase)"`
        );
      }
    }
  }
...
async function main() {
  ...
  console.log(
    `   Found ${Object.keys(cursorEnv).length} environment variables`
  );
  ...
  const matrix = generateSecretParityMatrix(
    cursorEnv,
    envLocal,
    vercelEnv,
    githubSecrets
  ...
  const reportPath = join(
    process.cwd(),
    '.cursor',
    'full-stack-smoke-test-report.md'
  );
  ...
  const jsonReportPath = join(
    process.cwd(),
    '.cursor',
    'full-stack-smoke-test-report.json'
  );

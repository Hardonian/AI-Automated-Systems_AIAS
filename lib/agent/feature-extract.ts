...
export async function computeSignalsForUser(
  userId: string,
  window: '1d' | '7d' | '30d' = '7d'
) {
  ...
  const checkoutDrops =
    events.filter(
      (e: { type: string; path?: string }) =>
        e.type === 'page_view' && e.path?.includes('checkout')
    ).length > 0 && !events.find((e: { type: string }) => e.type === 'purchase')
      ? 1
      : 0;
...
export async function persistSignals(
  userId: string,
  signals: { k: string; v: number; meta?: Record<string, unknown> }[],
  window: '1d' | '7d' | '30d' = '7d'
) {
  const supabase = createClient(
    process.env.NEXT_PUBLIC_SUPABASE_URL!,
...
const windowMs = (w: '1d' | '7d' | '30d') =>
  ({ '1d': 86400000, '7d': 604800000, '30d': 2592000000 })[w];
...
function countRageClicks(clicks: ClickEvent[]) {
  ...
  for (const arr of Object.values(byPath)) {
    ...
    for (let i = 3; i < arr.length; i++) {
      const current = arr[i];
      const prev = arr[i - 3];
      if (current && prev && current - prev <= 2000) {
        rage++;
      }
    }
  }

...
export const caseStudies: CaseStudy[] = [
  ...
  ...legacyCaseStudies.map((legacy) => {
    ...
    if (lowerSlug.includes("reach") || lowerArch.includes("reach"))
      involvedProjects.push(reachMeta);
    if (lowerSlug.includes("zeo") || lowerArch.includes("zeo"))
      involvedProjects.push(zeoMeta);
    if (lowerSlug.includes("settler") || lowerArch.includes("settler"))
      involvedProjects.push(settlerMeta);

    // If no projects explicitly found, assume all are involved as per AIAS ecosystem
    const metadataToUse =
      involvedProjects.length > 0
        ? involvedProjects
        : [reachMeta, zeoMeta, settlerMeta];

    return {
      slug: legacy.slug,
      title: legacy.title,
      problem: legacy.clientProblem,
      architecture: [legacy.architectureChosen, ...legacy.systemConstraints],

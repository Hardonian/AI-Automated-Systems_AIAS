...
function checkEnvVar(name: string): EnvVar {
  ...
  return {
    name,
    required: REQUIRED_ENV_VARS.includes(
      name as (typeof REQUIRED_ENV_VARS)[number]
    ),
...
function generateReport(): string {
  ...
  vercelVars.forEach(v => {
    ...
    report.push(
      `${status} ${v.name}: ${v.present ? v.value || 'set' : 'NOT SET'}`
    );
  });
  report.push('');
  ...
  requiredVars.forEach(v => {
    ...
    report.push(
      `${status} ${v.name} ${required}: ${v.present ? v.value || 'set' : 'NOT SET'}`
    );
  });
  ...
  recommendedVars.forEach(v => {
    const status = v.present ? '✓' : '○';
    report.push(
      `${status} ${v.name}: ${v.present ? v.value || 'set' : 'NOT SET (optional)'}`
    );
  });
  ...
  if (dbUrl) {
    ...
    report.push(
      `${isValid ? '✓' : '✗'} Database URL: ${isValid ? 'Valid format' : 'Invalid format'}`
    );
  ...
  report.push(
    `${supabaseUrl ? '✓' : '✗'} Supabase URL: ${supabaseUrl ? 'Set' : 'NOT SET'}`
  );
  report.push(
    `${supabaseKey ? '✓' : '✗'} Supabase Anon Key: ${supabaseKey ? 'Set' : 'NOT SET'}`
  );
  report.push(
    `${serviceKey ? '✓' : '✗'} Supabase Service Key: ${serviceKey ? 'Set' : 'NOT SET'}`
  );
  report.push('');
  ...
  const presentRequired = requiredVars.filter(
    v => v.required && v.present
  ).length;
  const allRequiredPresent = missingRequired.length === 0;

  report.push(
    `Required Variables: ${presentRequired}/${totalRequired} present`
  );
  report.push(
    `Status: ${allRequiredPresent ? '✓ READY FOR BUILD' : '✗ BUILD WILL FAIL'}`
  );
  report.push('');
...
try {
  ...
  const hasDatabase =
    process.env.DATABASE_URL || process.env.UPSTASH_POSTGRES_URL;

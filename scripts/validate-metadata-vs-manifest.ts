...
for (const route of INDEXABLE_ROUTE_MANIFEST) {
  ...
  if (!parsed.title)
    failures.push(
      `${route.path}: could not parse title from generateSEOMetadata config`,
    );
  if (!parsed.description)
    failures.push(
      `${route.path}: could not parse description from generateSEOMetadata config`,
    );
  if (!parsed.canonical)
    failures.push(
      `${route.path}: could not parse canonical from generateSEOMetadata config`,
    );

  if (parsed.title && parsed.title !== route.title) {
    failures.push(
      `${route.path}: title mismatch (page='${parsed.title}' manifest='${route.title}')`,
    );
  }
  if (parsed.description && parsed.description !== route.description) {
    failures.push(
      `${route.path}: description mismatch (page='${parsed.description}' manifest='${route.description}')`,
    );
  }
  if (parsed.canonical && parsed.canonical !== route.canonical) {
    failures.push(
      `${route.path}: canonical mismatch (page='${parsed.canonical}' manifest='${route.canonical}')`,
    );
  }
}
...
console.log(
  `Metadata-vs-manifest validation passed for ${INDEXABLE_ROUTE_MANIFEST.length} routes.`,
);

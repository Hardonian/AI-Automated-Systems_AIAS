  ...
  try {
    ...
    try {
      ...
      const { data: uploadData, error: uploadError } =
        await supabaseAdmin.storage.from('pdfs').upload(fileName, pdfBuffer, {
    ...
    try {
      ...
      if (resendKey || sendgridKey || (mailgunKey && mailgunDomain)) {
        const emailModule =
          await import('https://esm.sh/@/lib/email/sender.ts');
        const templateModule =
          await import('https://esm.sh/@/lib/email/templates.ts');
    ...
    try {
      const crmProvider = Deno.env.get('CRM_PROVIDER') as
        | 'hubspot'
        | 'salesforce'
        | undefined;
      if (crmProvider) {
        const crmModule =
          await import('https://esm.sh/@/lib/integrations/crm.ts');
        ...
        if (crmProvider === 'hubspot') {
          ...
          crmConfig.clientSecret =
            Deno.env.get('SALESFORCE_CLIENT_SECRET') || '';
          crmConfig.username = Deno.env.get('SALESFORCE_USERNAME') || '';
          crmConfig.password = Deno.env.get('SALESFORCE_PASSWORD') || '';
          crmConfig.securityToken =
            Deno.env.get('SALESFORCE_SECURITY_TOKEN') || '';

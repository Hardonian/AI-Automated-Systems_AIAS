...
class AIPerformanceWatcher {
  ...
  private async detectAnomalies(
    ...
    for (const [model, modelData] of Object.entries(modelMetrics)) {
      ...
      const accuracyScores = modelData
        .filter(m => m.accuracy_score !== undefined)
      ...
      if (maxTokens > avgTokens * 3) {
        // 3x average threshold
        anomalies.push({
          type: 'token_spike',
          severity: 'medium',
          model,
   ...
   * Group metrics by model
   */
  private groupMetricsByModel(
    metrics: AIPerformanceMetrics[]
  ): Record<string, AIPerformanceMetrics[]> {
...
### 📈 Model Performance
${Object.entries(report.model_performance)
  .map(
    ([model, perf]) => `
**${model}:**
...
*This issue was automatically generated by the AI Performance Watcher.*
  ...
  async run(): Promise<void> {
    try {
      ...
      console.log(
        `Anomalies: ${report.anomalies.length}, Critical: ${report.anomalies.filter(a => a.severity === 'critical').length}`
      );

...
function check(name: string, ok: boolean, message?: string, error?: string) {
  ...
  const icon = ok ? '✓' : '✗';
  console.log(
    `${icon} ${name}${message ? `: ${message}` : ''}${error ? ` - ${error}` : ''}`
  );
}
...
async function main() {
  ...
  try {
    ...
    check(
      'Supabase Auth',
      !error,
      error ? undefined : 'Service role access OK',
      error?.message
    );
  } catch (e: any) {
  ...
  try {
    ...
    if (error) {
      ...
      check(
        ...
        !publicBucket || !privateBucket
          ? 'Missing public or private bucket'
          : undefined
      );
    }
  } catch (e: any) {
    ...
    if (insertError) {
      ...
      if (readError) {
        ...
        check(
          ...
          false,
          undefined,
          'Anon can read - RLS may not be enforced'
        );
      }

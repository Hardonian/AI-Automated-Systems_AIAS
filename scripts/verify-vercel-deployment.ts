  ...
  try {
    ...
    if (config.outputDirectory) {
      if (config.outputDirectory === 'apps/web/.next') {
        addWarning(
          'outputDirectory points to apps/web/.next - verify this directory exists'
        );
...
function verifyPackageJson() {
  ...
  try {
    ...
    if (pkg.packageManager?.includes('pnpm')) {
      ...
      addWarning(
        `Package manager is ${pkg.packageManager || 'not specified'}, expected pnpm`
      );
    }
  } catch (error: any) {
...
function verifyPnpmLock() {
  ...
  if (existsSync(lockPath)) {
    ...
    addCheck(
      ...
      false,
      'pnpm-lock.yaml not found - required for frozen lockfile installs'
    );
  }
}
...
function verifyGitHubWorkflows() {
  ...
  const requiredWorkflows = ['frontend-deploy.yml', 'vercel-build-check.yml'];
...
async function main() {
  ...
  result.checks.forEach(check => {
    ...
    console.log(
      `${icon} ${check.name}${check.message ? `: ${check.message}` : ''}`
    );
  });

...
export function CaseStudyDashboard() {
  ...
  const getTierBadge = (tier: string) => {
    switch (tier) {
      ...
      case 'Starter':
        return (
          <Badge className='border-gray-500 text-gray-700' variant='outline'>
            Starter
          </Badge>
        );
      default:
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className='text-xs text-muted-foreground'>
                {summary.total > 0
                  ? ((summary.published / summary.total) * 100).toFixed(0)
                  : 0}
                % published
              </p>
            </CardContent>
          ...
          <Card>
            ...
            <CardContent>
              <div className='text-2xl font-bold'>
                {Object.keys(summary.byIndustry).length}
              </div>
              <p className='text-xs text-muted-foreground'>
        ...
        <TabsContent className='space-y-4' value='list'>
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Case studies for sales and marketing
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className='space-y-4'>
                {caseStudies.map(cs => (
                  <div key={cs.id} className='space-y-3 rounded-lg border p-4'>
                    ...
                    <div className='grid grid-cols-2 gap-4 text-sm md:grid-cols-4'>
                      <div>
                        ...
                        <p className='font-medium'>
                          {cs.results.quantitative.timeSaved.saved}
                        </p>
                      </div>
                      <div>
                        <p className='text-muted-foreground'>Value</p>
                        <p className='font-medium'>
                          {cs.results.quantitative.timeSaved.value}
                        </p>
                      </div>
                      <div>
                        <p className='text-muted-foreground'>ROI</p>
                        <p className='font-medium text-green-600'>
                          {cs.results.quantitative.roi.roi}
                        </p>
                      </div>
                      <div>
                        <p className='text-muted-foreground'>Payback</p>
                        <p className='font-medium'>
                          {cs.results.quantitative.roi.paybackPeriod}
                        </p>
                      </div>
                    </div>
                    ...
                    {cs.results.testimonial && (
                      <div className='border-l-4 border-blue-500 pl-4 italic'>
                        <p className='text-sm'>
                          "{cs.results.testimonial.quote}"
                        </p>
                        <p className='mt-1 text-xs text-muted-foreground'>
                          — {cs.results.testimonial.author},{' '}
                          {cs.results.testimonial.title},{' '}
                          {cs.results.testimonial.company}
                        </p>
                      </div>
                    )}
            ...
            <>
              ...
              <Card>
                ...
                <CardContent>
                  <ResponsiveContainer height={300} width='100%'>
                    <BarChart
                      data={Object.entries(summary.byIndustry).map(
                        ([industry, count]) => ({
                          industry,
                          count,
                        })

...
export const SubscriptionEnforcement: React.FC = () => {
    ...
    const gates: FeatureGate[] = [
      {
        ...
        reason:
          currentPlan.tier === 'starter'
            ? 'Available in Professional plan and above'
            : undefined,
      },
      {
        feature: 'API Access',
      ...
      },
      {
        ...
        reason:
          usageMetrics.workflows.used >= usageMetrics.workflows.limit
  ...
  const getUsageStatus = (used: number, limit: number) => {
    ...
    if (percentage >= 100) {
      return { status: 'limit_reached', color: 'text-red-600' };
    }
    if (percentage >= 90) {
      return { status: 'near_limit', color: 'text-yellow-600' };
    }
  ...
  return (
    <div className='space-y-6'>
      <div className='flex items-center justify-between'>
        <div>
          ...
          <p className='mt-1 text-gray-600'>
            Monitor usage, manage limits, and control feature access
          </p>
        </div>
        ...
        <CardContent>
          <div className='flex items-center justify-between'>
            <div className='flex items-center gap-4'>
              <div
                className={`h-12 w-12 ${currentPlan.color} flex items-center justify-center rounded-lg text-white`}
              >
                {currentPlan.icon}
              </div>
              <div>
                ...
                <p className='text-gray-600'>
                  ${currentPlan.price.monthly}/month or $
                  {currentPlan.price.yearly}/year
                </p>
              </div>
            </div>
      ...
      <div className='grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3'>
          ...
          <CardContent className='p-6'>
            <div className='mb-4 flex items-center justify-between'>
              ...
              <Badge
                className={
                  getUsageStatus(
                    usageMetrics.workflows.used,
                    usageMetrics.workflows.limit
                  ).color
                }
              >
                {usageMetrics.workflows.used}/
                {usageMetrics.workflows.limit === -1
                  ? '∞'
                  : usageMetrics.workflows.limit}
              </Badge>
            </div>
            ...
            <div className='text-sm text-gray-600'>
              {usageMetrics.workflows.limit === -1
                ? 'Unlimited'
                : `${usageMetrics.workflows.used} of ${usageMetrics.workflows.limit} used`}
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className='p-6'>
            <div className='mb-4 flex items-center justify-between'>
              ...
              <Badge
                className={
                  getUsageStatus(
                    usageMetrics.executions.used,
                    usageMetrics.executions.limit
                  ).color
                }
              >
                {usageMetrics.executions.used.toLocaleString()}/
                {usageMetrics.executions.limit.toLocaleString()}
              </Badge>
            </div>
            <Progress
              className='mb-2 h-2'
            ...
            />
            <div className='text-sm text-gray-600'>
              {usageMetrics.executions.used.toLocaleString()} of{' '}
              {usageMetrics.executions.limit.toLocaleString()} this month
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className='p-6'>
            ...
            />
            <div className='text-sm text-gray-600'>
              {usageMetrics.storage.used}GB of {usageMetrics.storage.limit}GB
              used
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className='p-6'>
            ...
            <div className='text-sm text-gray-600'>
              {usageMetrics.users.limit === -1
                ? 'Unlimited'
                : `${usageMetrics.users.used} of ${usageMetrics.users.limit} used`}
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className='p-6'>
            ...
            />
            <div className='text-sm text-gray-600'>
              {usageMetrics.apiCalls.used.toLocaleString()} of{' '}
              {usageMetrics.apiCalls.limit.toLocaleString()} this month
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className='p-6'>
            <div className='mb-4 flex items-center justify-between'>
              ...
              <Badge
                className={
                  getUsageStatus(
                    usageMetrics.aiAgents.used,
                    usageMetrics.aiAgents.limit
                  ).color
                }
              >
                {usageMetrics.aiAgents.used}/
                {usageMetrics.aiAgents.limit === -1
                  ? '∞'
                  : usageMetrics.aiAgents.limit}
              </Badge>
            </div>
            ...
            <div className='text-sm text-gray-600'>
              {usageMetrics.aiAgents.limit === -1
                ? 'Unlimited'
                : `${usageMetrics.aiAgents.used} of ${usageMetrics.aiAgents.limit} used`}
            </div>
          </CardContent>
        </Card>
      </div>
            ...
            {subscriptionPlans.map(plan => (
              ...
              >
                <div className='mb-4 text-center'>
                  <div
                    className={`h-12 w-12 ${plan.color} mx-auto mb-3 flex items-center justify-center rounded-lg text-white`}
                  >
                    {plan.icon}
                  </div>
      ...
      {/* Usage Alerts */}
      {Object.entries(usageMetrics).some(
        ([_, metric]) =>
        ...
        <Alert>
          ...
          <AlertDescription>
            You're approaching usage limits on several features. Consider

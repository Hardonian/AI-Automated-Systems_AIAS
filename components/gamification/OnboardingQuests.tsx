...
export default function OnboardingQuests() {
  ...
  async function loadQuests() {
    ...
    if (data) {
      setQuests(data);
      setCompletedCount(
        data.filter((q: { completed_at?: string | null }) => q.completed_at)
          .length
      );
    }
  }
  ...
  const completedTypes = new Set(
    quests.filter(q => q.completed_at).map(q => q.quest_type)
  );
    ...
    <div className='space-y-4'>
      <div className='flex items-center justify-between'>
        ...
        <div className='text-xs text-muted-foreground'>
          {completedCount}/{ONBOARDING_QUESTS.length} completed
        </div>
      </div>
          ...
          return (
            ...
            >
              ...
              {completed ? (
                ...
                <button
                  className='text-primary-fg h-8 rounded-lg bg-primary px-3 text-xs'

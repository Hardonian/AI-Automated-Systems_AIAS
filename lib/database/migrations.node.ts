...
async function runMigrationsViaSupabaseCLI(): Promise<MigrationResult> {
  ...
  try {
    ...
    try {
      ...
      console.log(
        '⚠️  Supabase CLI not found, falling back to direct SQL application'
      );
    ...
    if (process.env.SUPABASE_ACCESS_TOKEN) {
      execSync(
        `echo "${process.env.SUPABASE_ACCESS_TOKEN}" | supabase login --token -`,
        {
          stdio: 'pipe',
          env: { ...process.env },
        }
    ...
    console.log(
      '⚠️  Supabase CLI method failed, falling back to direct SQL:',
      errorMessage
    );
    throw error;
  }
  ...
  for (const migration of pendingMigrations) {
    ...
    if (migrationResult.success) {
      ...
      if (process.env.NODE_ENV === 'production') {
        console.error(
          `❌ Stopping migration process due to failure: ${migration.name}`
        );
        break;
      }
    }
...
export async function runMigrationsOnStartup(): Promise<MigrationResult> {
  ...
  if (!dbUrl) {
    ...
    console.warn(
      '   Set SUPABASE_DB_URL, DATABASE_URL, or DIRECT_URL to enable migrations'
    );
    return { success: true, applied: 0, skipped: 0, failed: 0, errors: [] };
  }
  ...
  try {
    ...
    for (const table of criticalTables) {
      ...
      if (error && error.code !== 'PGRST116') {
        ...
        console.warn(
          `⚠️  Critical table check failed for ${table}:`,
          error.message
        );
        if (process.env.NODE_ENV === 'production') {

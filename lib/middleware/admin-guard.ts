...
export async function adminGuard(
  ...
  try {
    ...
    if (profileError) {
      logger.warn(
        'Admin role lookup failed in middleware',
        profileError as any,
        {
          path,
          userId: user.id,
    ...
    if (!isAdmin) {
      const res = NextResponse.redirect(
        new URL('/signin?error=admin_access_required', request.url)
      );
      addSecurityHeaders(res);
      return res;
    }
...
export async function financialAdminGuard(
  ...
  if (
    !path.startsWith('/admin/financial') &&
    !path.startsWith('/api/admin/financial')
  ) {
    return null;
  }
  ...
  try {
    ...
    if (profileError) {
      logger.warn(
        'Financial admin role lookup failed in middleware',
        profileError as any,
        {
          path,
          userId: user.id,
    ...
    const hasFinancialAccess =
      role === 'financial_admin' || role === 'super_admin';

    if (!hasFinancialAccess) {
      const res = NextResponse.redirect(
        new URL('/admin?error=financial_access_required', request.url)
      );
      addSecurityHeaders(res);
      return res;
    }

...
class EnhancedSmokeTestRunner extends SmokeTestRunner {
  ...
  private async testCircuitBreaker(): Promise<void> {
    const response = await fetch(
      `${this.baseUrl}/api/circuit-breaker/metrics`,
      {
        headers: {
  ...
  private async testMonitoringDashboard(): Promise<void> {
    ...
    if (!response.ok) {
      throw new Error(
        `Monitoring dashboard endpoint failed: ${response.status}`
      );
    }
  ...
  async runEnhanced(): Promise<void> {
    ...
    // Run enhanced tests
    await this.runTest('Circuit Breaker Metrics', () =>
      this.testCircuitBreaker()
    );
    await this.runTest('Cache Analytics', () => this.testCacheAnalytics());
    await this.runTest('Monitoring Dashboard', () =>
      this.testMonitoringDashboard()
    );

...
test.describe('Visual Regression - Hero breakpoints', () => {
  test('hero spacing remains stable across viewport breakpoints', async ({ page }, testInfo) => {
    ...
    const impactCardContainer = hero.getByTestId('hero-impact-card-container').first();
    await expect(impactCardContainer).toBeVisible();
    await expect(impactCardContainer).toHaveScreenshot('hero-impact-card-container.png', {
      animations: 'disabled',
      scale: 'css',
    });

...
serve(async req => {
  ...
  try {
    ...
    const tenantId =
      urlObj.searchParams.get('tenantId') || req.headers.get('x-tenant-id');
    ...
    switch (method) {
      ...
      case 'POST':
        if (resource === 'reports' && action === 'generate') {
          ...
          const body = await req.json();
          return await createAnalyticsAlert(
            supabaseClient,
            tenantId,
            body,
            user.id
...
async function getRevenueAnalytics(
  ...
  const analytics: RevenueAnalytics = {
    ...
    revenueByPeriod: groupRevenueByPeriod(
      revenueData || [],
      query.groupBy || 'day'
    ),
...
async function getAnalyticsDashboard(
  ...
  const dashboard = {
    ...
    realTime: {
      activeUsers:
        realTimeMetrics?.reduce((sum, item) => sum + item.usage_count, 0) || 0,
...
async function generateCustomReport(
  ...
  setTimeout(async () => {
    const reportData = await executeCustomQuery(
      supabaseClient,
      tenantId,
      body.query,
      body.parameters
...
async function createAnalyticsAlert(
  supabaseClient: any,
  tenantId: string,
  body: any,
  userId: string
...
function getPreviousPeriodDateRange(query: AnalyticsQuery): {
  start: string;
  end: string;
} {
  const currentRange = getDateRange(query);
  const duration =
    new Date(currentRange.end).getTime() -
    new Date(currentRange.start).getTime();
...
function getTopUsedFeatures(
  usageData: any[]
): Array<{ feature: string; usage: number; percentage: number }> {
...
function calculateAverageResponseTime(apiData: any[]): number {
  ...
  const totalTime = apiData.reduce(
    (sum, item) => sum + (item.response_time || 0),
    0
  );
...
function calculateSuccessRate(apiData: any[]): number {
  ...
  const successCount = apiData.filter(
...
function getPerformanceByService(
  apiData: any[]
): Array<{ service: string; responseTime: number; successRate: number }> {
  const serviceMap: Record<
    string,
    { totalTime: number; count: number; successCount: number }
  > = {};
...
function getPerformanceTrends(
  apiData: any[],
  groupBy: string
): Array<{ period: string; responseTime: number; successRate: number }> {
...
async function getRecommendations(
  ...
  if (revenue.revenueGrowth < 0) {
    recommendations.push({
      ...
      description:
        'Your revenue has decreased compared to the previous period. Consider reviewing your pricing strategy.',
  ...
  if (performance.successRate < 95) {
    recommendations.push({
      ...
      description:
        'Your API success rate is below 95%. Consider investigating error causes.',
  ...
  if (business.churnRate > 10) {
    recommendations.push({
      ...
      description:
        'Your churn rate is above 10%. Focus on customer retention strategies.',

  ...
  return (
    <div className="relative h-64 w-full overflow-hidden rounded-2xl bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-6 shadow-2xl">
      ...
      {[
        ...
        const radius = 80;
        const x =
          Math.round(Math.cos((node.angle * Math.PI) / 180) * radius * 100) /
          100;
        const y =
          Math.round(Math.sin((node.angle * Math.PI) / 180) * radius * 100) /
          100;
        return (
          <motion.div
            key={node.label}
          ...
          >
            <div
              className={`flex items-center gap-2 rounded-full ${node.color} px-3 py-1.5 text-xs font-bold text-white shadow-lg`}
            >
              {node.label}
            </div>
          </motion.div>
      ...
      >
        ...
        {[0, 72, 144, 216, 288].map((angle, i) => {
          ...
          const x2 =
            Math.round(
              (200 + Math.cos((angle * Math.PI) / 180) * radius) * 100,
            ) / 100;
          const y2 =
            Math.round(
              (128 + Math.sin((angle * Math.PI) / 180) * radius) * 100,
            ) / 100;
          return (
            <motion.line
              key={angle}

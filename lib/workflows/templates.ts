...
export const workflowTemplates: WorkflowTemplate[] = [
  {
    ...
    description:
      'Send a Slack notification when a new order is placed in Shopify',
    category: 'ecommerce',
    icon: '🛍️',
  ...
  },
  {
    ...
    description:
      'Automatically process orders: create invoice, send confirmation, update inventory',
    category: 'ecommerce',
    icon: '⚙️',
  ...
  for (const step of template.steps) {
    if (step.requiredFields) {
      for (const field of step.requiredFields) {
        const stepConfig = config[step.id] as

...
export function FunnelChart({ data }: FunnelChartProps) {
  const chartData = [
    { stage: 'Signups', count: data.signups, percentage: 100 },
    {
      ...
      percentage:
        data.signups > 0 ? (data.integrations / data.signups) * 100 : 0,
    },
    {
      stage: 'Workflows',
      count: data.workflows,
      percentage:
        data.integrations > 0 ? (data.workflows / data.integrations) * 100 : 0,
    },
    {
      stage: 'Activations',
      count: data.activations,
      percentage:
        data.workflows > 0 ? (data.activations / data.workflows) * 100 : 0,
    },
  ];

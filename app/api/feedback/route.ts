...
export const POST = createValidatedRoute(
  feedbackSchema,
  async (data, _req: NextRequest): Promise<NextResponse<FeedbackResponse>> => {
    try {
      ...
      const { error } = await retry(
        ...
        },
        {
          ...
          initialDelayMs: 1000,
          onRetry: (attempt, err) => {
            logger.warn(`Retrying feedback insert (attempt ${attempt})`, {
              error: err.message,
            });
          },
        }
      ...
      if (error) {
        ...
        logger.error('Feedback submission error', systemError, {
          userId,
          rating,
        });
        ...
        return NextResponse.json(
          {
            ...
            details: formatted.details?.originalError as string,
          },
          { status: formatted.statusCode }
        );
      }

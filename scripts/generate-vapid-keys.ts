...
function generateVAPIDKeys() {
  ...
  const _publicKey = curve
    .getPublicKey('base64')
    .replace(/\+/g, '-')
    .replace(/\//g, '_')
    .replace(/=/g, '');
  const _privateKey = curve
    .getPrivateKey('base64')
    .replace(/\+/g, '-')
    .replace(/\//g, '_')
    .replace(/=/g, '');

  // Convert to URL-safe base64
  const publicKeyBase64 = Buffer.from(curve.getPublicKey()).toString(
    'base64url'
  );
  const privateKeyBase64 = Buffer.from(curve.getPrivateKey()).toString(
    'base64url'
  );

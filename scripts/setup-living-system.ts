...
async function runSetup() {
  ...
  console.log(
    '⚠️  Have you applied the migration? (This script will verify next)'
  );
  ...
  try {
    ...
    console.error(
      '   Please apply the migration first (see instructions above).\n'
    );
    console.log(
      '   You can run this script again after applying the migration.\n'
    );
    process.exit(1);
  }
  ...
  console.log(
    'This will create a test user and verify the complete data flow.'
  );
  ...
  try {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(
          `   All Cylinders Firing: ${data.allCylindersFiring ? 'Yes ✓' : 'No ⚠️'}\n`
        );
      } else {
      ...
      console.log(
        'ℹ️  Could not reach health endpoint (server may not be running)'
      );
      console.log(
        '   This is okay - you can test it once your app is deployed.\n'
      );
    }
  } catch (error) {
    console.log('ℹ️  Health check skipped (node-fetch not available)');
    console.log(
      '   You can test manually: curl http://localhost:3000/api/status/health\n'
    );
  }

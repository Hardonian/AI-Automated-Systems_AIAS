...
async function applyMigration() {
  ...
  try {
    ...
    for (let i = 0; i < statements.length; i++) {
      ...
      try {
        ...
        if (i === 0) {
          ...
          console.log(
            'ℹ️  To apply this migration, use one of the following methods:\n'
          );
    ...
    const verifyQueries = [
      {
        ...
        query:
          "SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'activity_log')",
      },
      {
        name: 'positioning_feedback table',
        query:
          "SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'positioning_feedback')",
      },
      {
        name: 'kpi_new_users_week view',
        query:
          "SELECT EXISTS (SELECT FROM information_schema.views WHERE table_schema = 'public' AND table_name = 'kpi_new_users_week')",
      },
      {
        name: 'kpi_avg_post_views view',
        query:
          "SELECT EXISTS (SELECT FROM information_schema.views WHERE table_schema = 'public' AND table_name = 'kpi_avg_post_views')",
      },
      {
        name: 'kpi_actions_last_hour view',
        query:
          "SELECT EXISTS (SELECT FROM information_schema.views WHERE table_schema = 'public' AND table_name = 'kpi_actions_last_hour')",
      },
    ];
    ...
    console.log(
      '   2. Run the verification script: tsx scripts/verify-migration.ts'
    );
    console.log(
      '   3. Test the health endpoint: curl http://localhost:3000/api/status/health\n'
    );

...
async function main() {
  ...
  checks.push(
    checkFile(
      'supabase/migrations/20250123000000_performance_metrics.sql',
      'Migration file'
    )
  );
  checks.push(checkFile('app/api/metrics/route.ts', 'Metrics API endpoint'));
  checks.push(
    checkFile('app/api/telemetry/route.ts', 'Telemetry API endpoint')
  );
  ...
  checks.push(
    checkFile('scripts/generate-performance-report.ts', 'Report generator')
  );

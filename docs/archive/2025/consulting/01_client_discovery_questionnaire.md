> Archived on 2025-11-12. Superseded by: (see docs/final index)

# Client Discovery Questionnaire

## Comprehensive Business & Automation Assessment

**Purpose:** Understand client's business, current processes, pain points, and automation opportunities  
**Duration:** 60-90 minutes  
**Use Before:** Automation Readiness Assessment, Strategy Development

---

## 📋 PRE-INTERVIEW PREPARATION

### Client Information

- **Company Name:** \***\*\*\*\*\***\_\_\_\***\*\*\*\*\***
- **Industry:** \***\*\*\*\*\***\_\_\_\***\*\*\*\*\***
- **Company Size:** [ ] Solo [ ] 2-10 [ ] 11-50 [ ] 51-200 [ ] 200+
- **Primary Location:** \***\*\*\*\*\***\_\_\_\***\*\*\*\*\***
- **Other Locations:** \***\*\*\*\*\***\_\_\_\***\*\*\*\*\***
- **Annual Revenue:** \***\*\*\*\*\***\_\_\_\***\*\*\*\*\***
- **Interview Date:** \***\*\*\*\*\***\_\_\_\***\*\*\*\*\***
- **Consultant:** \***\*\*\*\*\***\_\_\_\***\*\*\*\*\***

---

## 1. BUSINESS OVERVIEW

### 1.1 Core Business

- **What does your company do?**

  ***

  ***

- **What are your primary products/services?**

  ***

  ***

- **Who are your target customers?**

  ***

  ***

- **What makes your business unique?**
  ***
  ***

### 1.2 Business Goals (12-24 months)

- **What are your top 3 business objectives?**
  1. ***
  2. ***
  3. ***

- **What are your biggest challenges right now?**
  1. ***
  2. ***
  3. ***

- **What growth plans do you have?**
  ***
  ***

---

## 2. CURRENT PROCESSES & OPERATIONS

### 2.1 Daily Operations

- **What are your most time-consuming daily tasks?**
  1. ***
  2. ***
  3. ***

- **What repetitive tasks do you or your team perform?**
  1. ***
  2. ***
  3. ***

- **How many hours per week do you spend on administrative tasks?**
  [ ] 0-5 hours [ ] 6-10 hours [ ] 11-20 hours [ ] 20+ hours

### 2.2 Pain Points

- **What processes are most frustrating or error-prone?**
  1. ***
  2. ***
  3. ***

- **Where do you experience the most delays or bottlenecks?**
  1. ***
  2. ***
  3. ***

- **What manual work would you eliminate if possible?**
  1. ***
  2. ***
  3. ***

### 2.3 Current Tools & Systems

- **What software/tools do you currently use?**
  - CRM: \***\*\*\*\*\***\_\_\_\***\*\*\*\*\***
  - Accounting: \***\*\*\*\*\***\_\_\_\***\*\*\*\*\***
  - Email: \***\*\*\*\*\***\_\_\_\***\*\*\*\*\***
  - Project Management: \***\*\*\*\*\***\_\_\_\***\*\*\*\*\***
  - Other: \***\*\*\*\*\***\_\_\_\***\*\*\*\*\***

- **Do these tools integrate with each other?**
  [ ] Yes [ ] No [ ] Somewhat [ ] Don't know

- **What's your biggest challenge with current tools?**
  ***
  ***

---

## 3. SYSTEMS & PRODUCTIVITY ANALYSIS

### 3.1 Systems Thinking Questions

**We believe productivity requires multiple approaches, not just automation. Let's understand your system holistically:**

- **How do you currently think about productivity?**
  [ ] Automation-focused [ ] Process-focused [ ] People-focused [ ] Multi-faceted [ ] Not sure

- **Have you analyzed productivity challenges from multiple perspectives?**
  [ ] Yes [ ] No [ ] Somewhat
  **If yes, what perspectives did you consider?**

  ***

- **What do you think are the root causes of productivity challenges?** (Not just symptoms)
  1. ***
  2. ***
  3. ***

### 3.2 Multi-Dimensional Assessment

**Process Dimension:**

- **Are your processes optimized, or are they just "how things have always been done"?**
  [ ] Optimized [ ] Traditional [ ] Inefficient [ ] Don't know

- **Where do you see process bottlenecks or inefficiencies?**
  ***
  ***

**Technology Dimension:**

- **Do your tools work well together, or do you have integration challenges?**

  ***

  ***

- **Are you using tools to their full potential?**
  [ ] Yes [ ] No [ ] Not sure

**People Dimension:**

- **Do your team members have the skills needed for their roles?**
  [ ] Yes [ ] No [ ] Some gaps
  **What gaps exist?**

  ***

- **How would you describe your organizational culture regarding productivity?**
  ***
  ***

**Data Dimension:**

- **Do you make decisions based on data or intuition?**
  [ ] Mostly data [ ] Mix [ ] Mostly intuition

- **Do you have the data you need when you need it?**
  [ ] Yes [ ] No [ ] Sometimes

**Systems Architecture:**

- **How well do your systems integrate with each other?**
  [ ] Seamlessly [ ] Somewhat [ ] Poorly [ ] Not at all

- **Can your current systems scale with growth?**
  [ ] Yes [ ] No [ ] Not sure

**Automation Dimension:**

- **Do you currently use any automation?**
  [ ] Yes [ ] No [ ] Limited
  **If yes, what's working/not working?**

  ***

- **What's your view on automation?**
  [ ] Solution to everything [ ] One tool among many [ ] Overhyped [ ] Not sure

### 3.3 Technical Comfort

- **How comfortable are you with technology?**
  [ ] Very comfortable [ ] Somewhat comfortable [ ] Uncomfortable

- **Do you have technical resources on your team?**
  [ ] Yes [ ] No
  **If yes, what capacity?**
  ***

---

## 4. DATA & INFORMATION

### 4.1 Data Management

- **Where is your business data stored?**
  [ ] Spreadsheets [ ] Databases [ ] Cloud [ ] Paper files [ ] Mixed

- **How do you track important metrics/KPIs?**

  ***

  ***

- **How often do you update this data?**
  [ ] Real-time [ ] Daily [ ] Weekly [ ] Monthly [ ] Rarely

### 4.2 Data Integration

- **Do you need to move data between systems?**
  [ ] Yes [ ] No
  **If yes, how often?**
  [ ] Constantly [ ] Daily [ ] Weekly [ ] Monthly

- **What data integration challenges do you face?**
  ***
  ***

---

## 5. STAKEHOLDERS & TEAM

### 5.1 Team Structure

- **Who would be involved in automation projects?**
  - Decision Maker: \***\*\*\*\*\***\_\_\_\***\*\*\*\*\***
  - Primary User: \***\*\*\*\*\***\_\_\_\***\*\*\*\*\***
  - Technical Contact: \***\*\*\*\*\***\_\_\_\***\*\*\*\*\***
  - Other Stakeholders: \***\*\*\*\*\***\_\_\_\***\*\*\*\*\***

### 5.2 Change Management

- **How does your team typically adapt to new tools/processes?**

  ***

  ***

- **What concerns do stakeholders have about automation?**
  1. ***
  2. ***
  3. ***

---

## 6. BUDGET & TIMELINE

### 6.1 Budget

- **What's your budget for automation/operational improvements?**
  [ ] $0-1,000 [ ] $1,000-5,000 [ ] $5,000-10,000 [ ] $10,000+ [ ] TBD

- **How do you measure ROI for technology investments?**
  ***
  ***

### 6.2 Timeline

- **What's your ideal timeline for seeing results?**
  [ ] 1-3 months [ ] 3-6 months [ ] 6-12 months [ ] 12+ months

- **Are there any deadlines or milestones we should consider?**
  ***
  ***

---

## 7. SUCCESS CRITERIA

### 7.1 Desired Outcomes

- **What would success look like 6 months from now?**

  ***

  ***

- **What metrics would you use to measure success?**
  1. ***
  2. ***
  3. ***

### 7.2 Priorities

- **What's most important to you?**
  [ ] Time savings [ ] Cost reduction [ ] Error reduction [ ] Scalability [ ] Other: \***\*\_\*\***

---

## 8. NEXT STEPS

### 8.1 Immediate Priorities

- **What's the most urgent process you'd like to automate?**

  ***

  ***

- **What would have the biggest impact if automated today?**
  ***
  ***

### 8.2 Questions for Consultant

- **What questions do you have about automation?**
  1. ***
  2. ***
  3. ***

---

## 📝 CONSULTANT NOTES

### Key Insights

---

---

---

### Red Flags

---

---

---

### Opportunities

---

---

---

### Recommended Next Steps

1. ***
2. ***
3. ***

---

## ✅ FOLLOW-UP ACTIONS

- [ ] Schedule Automation Readiness Assessment
- [ ] Request access to current systems/tools
- [ ] Gather process documentation
- [ ] Identify additional stakeholders to interview
- [ ] Prepare discovery summary report

---

**Document Version:** 1.0  
**Last Updated:** January 2025

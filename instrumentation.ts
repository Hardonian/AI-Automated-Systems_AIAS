...
async function getRunMigrations() {
  ...
  try {
    const migrations = await import('@/lib/database/migrations');
...
function shouldSkipMigrations(): boolean {
  ...
  if (process.env.CI === 'true' || process.env.GITHUB_ACTIONS === 'true') {
    ...
    if (process.env.GITHUB_EVENT_NAME === 'pull_request') {
      console.log(
        '⏭️  [Instrumentation] Skipping migrations - PR event detected'
      );
      return true;
    }
    ...
    if (
      ...
      console.log(
        '⏭️  [Instrumentation] Skipping migrations - Main branch merge detected'
      );
      return true;
    }
  }
  ...
  if (process.env.SKIP_MIGRATIONS === 'true') {
    console.log(
      '⏭️  [Instrumentation] Skipping migrations - SKIP_MIGRATIONS=true'
    );
    return true;
  }
...
export async function register() {
  if (process.env.NEXT_RUNTIME === 'nodejs') {
    ...
    try {
      console.log(
        '🔄 [Instrumentation] Checking for pending database migrations...'
      );
      ...
      console.error(
        '⚠️  [Instrumentation] Server will continue, but migrations should be applied manually'
      );

      // In production, we might want to fail fast
      if (
        process.env.NODE_ENV === 'production' &&
        process.env.FAIL_ON_MIGRATION_ERROR === 'true'
      ) {
        console.error(
          '💥 [Instrumentation] Failing fast due to migration error in production'
        );
        process.exit(1);
      }
    }

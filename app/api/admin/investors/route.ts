...
export async function GET(request: NextRequest) {
  return createGETHandler(async () => {
    ...
    const investors: Investor[] = [
      {
        ...
        notes:
          'Very interested, asked for customer references. Follow up in 1 week.',
        dealStatus: 'pending',
      },
      {
        id: 'inv_002',
    ...
    const contacted = investors.filter(
      i => i.status !== 'not_contacted'
    ).length;
    const meetingsScheduled = investors.filter(
      i => i.status === 'meeting_scheduled' || i.status === 'meeting_completed'
    ).length;
    const meetingsCompleted = investors.filter(
      i => i.status === 'meeting_completed'
    ).length;
    const interested = investors.filter(
      i => i.status === 'interested' || i.status === 'committed'
    ).length;
    ...
    const byStatus = {
      ...
      meeting_scheduled: investors.filter(i => i.status === 'meeting_scheduled')
        .length,
      meeting_completed: investors.filter(i => i.status === 'meeting_completed')
        .length,

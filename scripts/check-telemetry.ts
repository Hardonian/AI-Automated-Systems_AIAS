...
async function checkFile(filePath: string): Promise<TelemetryCheckResult> {
  ...
  const hasTelemetry =
    content.includes('telemetry.track') ||
    content.includes('telemetry.trackPerformance');
  const hasPerformanceTracking =
    content.includes('trackPerformance') || content.includes('X-Response-Time');
...
async function main() {
  ...
  console.log(
    `⚠️  Found ${filesWithIssues.length} files with missing telemetry:\n`
  );

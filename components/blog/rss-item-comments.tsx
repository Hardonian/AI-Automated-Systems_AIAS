...
interface RSSItemCommentsProps {
  itemId: string;
  itemTitle: string;
}

export function RSSItemComments({ itemId, itemTitle }: RSSItemCommentsProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/blog/rss-comments', {
        method: 'POST',
        headers: {
    ...
    <section className='mt-12'>
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Share your thoughts on this news. All comments are moderated by AI
            using systems thinking principles to ensure quality, relevance, and
        ...
        <CardContent className='space-y-6'>
          ...
          <form className='space-y-4' onSubmit={handleSubmit}>
            ...
            <div>
              ...
              <p className='mt-1 text-xs text-muted-foreground'>
                💡 Tip: Comments that demonstrate systems thinking, multiple
                perspectives, or constructive dialogue are highly valued!
              </p>
            </div>
          ...
          <div className='mt-8 space-y-4'>
            <h3 className='font-semibold'>
              Discussion ({comments.filter(c => c.status === 'approved').length}
              )
            </h3>
            {comments.filter(c => c.status === 'approved').length === 0 ? (
              <p className='text-sm text-muted-foreground'>
                No comments yet. Be the first to share your systems thinking
                perspective!
              </p>
            ) : (
              comments

import type { Metadata } from 'next';
import Link from 'next/link';
import { notFound } from 'next/navigation';

import { RSSItemComments } from '@/components/blog/rss-item-comments';
import { Button } from '@/components/ui/button';
import {
  Card,
  CardContent,
  CardDescription,
  CardHeader,
  CardTitle,
} from '@/components/ui/card';
import {
  generateEditorialTake,
  generateQuickTake,
} from '@/lib/blog/rss-editorial';
import { RSSFeedItem } from '@/lib/blog/rss-feed';

interface PageProps {
  params: Promise<{
    id: string;
  }>;
}

// Mock function - in production, fetch from database/API
function getRSSItem(id: string): RSSFeedItem | null {
  // TODO: Fetch from database
  // For now, return mock data
  return {
    id,
    title: 'OpenAI Releases GPT-5 with Systems Thinking Capabilities',
    link: 'https://example.com/news/gpt5',
    description:
      "OpenAI's latest model includes systems thinking analysis features, enabling multi-perspective problem solving.",
    pubDate: new Date().toISOString(),
    source: 'TechCrunch AI',
    category: 'AI',
    relevance: 'high',
    systemsThinkingAngle: 'Directly relates to systems thinking concepts',
    perspectives: ['Technology', 'Systems', 'Automation'],
    discussionEnabled: true,
  };
}

export async function generateMetadata({
  params,
}: PageProps): Promise<Metadata> {
  const { id } = await params;
  const item = getRSSItem(id);

  if (!item) {
    return {
      title: 'News Item Not Found',
    };
  }

  return {
    title: `${item.title} | AIAS Platform Editorial & Discussion`,
    description: item.description,
  };
}

export default async function RSSItemPage({ params }: PageProps) {
  const { id } = await params;
  const item = getRSSItem(id);

  if (!item) {
    notFound();
  }

  const editorial = generateEditorialTake(item);
  const quickTake = generateQuickTake(item);

  return (
    <div className='container max-w-4xl py-16'>
      {/* Back to News Feed */}
      <div className='mb-6'>
        <Link
          className='text-sm text-muted-foreground hover:underline'
          href='/rss-news'
        >
          ← Back to AI & Tech News Feed
        </Link>
      </div>

      {/* Original News Item */}
      <Card className='mb-8'>
        <CardHeader>
          <div className='mb-2 flex items-center gap-2'>
            <span className='rounded-full bg-primary/10 px-3 py-1 text-sm font-medium text-primary'>
              {item.source}
            </span>
            {item.category && (
              <span className='rounded-full bg-muted px-3 py-1 text-sm text-muted-foreground'>
                {item.category}
              </span>
            )}
            {item.relevance && (
              <span
                className={`rounded-full px-3 py-1 text-sm ${
                  item.relevance === 'high'
                    ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200'
                    : item.relevance === 'medium'
                      ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200'
                      : 'bg-gray-100 text-gray-800 dark:bg-gray-900 dark:text-gray-200'
                }`}
              >
                {item.relevance.charAt(0).toUpperCase() +
                  item.relevance.slice(1)}{' '}
                Relevance
              </span>
            )}
          </div>
          <CardTitle className='mb-2 text-3xl'>{item.title}</CardTitle>
          <CardDescription className='text-base'>
            {item.description}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className='mb-4 flex items-center justify-between text-sm text-muted-foreground'>
            <time dateTime={item.pubDate}>
              {new Date(item.pubDate).toLocaleDateString('en-CA', {
                year: 'numeric',
                month: 'long',
                day: 'numeric',
                hour: 'numeric',
                minute: 'numeric',
              })}
            </time>
            <a
              className='text-primary hover:underline'
              href={item.link}
              rel='noopener noreferrer'
              target='_blank'
            >
              Read Original Article →
            </a>
          </div>

          {/* Systems Thinking Analysis */}
          {item.systemsThinkingAngle && (
            <div className='mb-4 rounded-r-lg border-l-4 border-primary bg-primary/10 p-4'>
              <p className='mb-2 font-semibold'>🧠 Systems Thinking Analysis</p>
              <p className='text-sm text-muted-foreground'>
                {item.systemsThinkingAngle}
              </p>
              {item.perspectives && item.perspectives.length > 0 && (
                <div className='mt-2'>
                  <p className='mb-1 text-xs font-semibold'>Perspectives:</p>
                  <div className='flex flex-wrap gap-2'>
                    {item.perspectives.map(perspective => (
                      <span
                        key={perspective}
                        className='rounded bg-primary/20 px-2 py-1 text-xs text-primary'
                      >
                        {perspective}
                      </span>
                    ))}
                  </div>
                </div>
              )}
            </div>
          )}
        </CardContent>
      </Card>

      {/* Editorial Take */}
      <Card className='mb-8 border-primary/20 bg-primary/5'>
        <CardHeader>
          <CardTitle className='flex items-center gap-2'>
            ✍️ Editorial Take
          </CardTitle>
          <CardDescription>
            Our quick editorialized perspective on this news
          </CardDescription>
        </CardHeader>
        <CardContent className='space-y-4'>
          {/* Quick Take */}
          <div className='rounded-r-lg border-l-4 border-primary bg-primary/10 p-4'>
            <p className='mb-2 text-sm font-semibold'>Quick Take:</p>
            <p className='text-muted-foreground'>{quickTake}</p>
          </div>

          {/* Full Editorial */}
          <div className='prose prose-sm max-w-none dark:prose-invert'>
            <p className='leading-relaxed text-muted-foreground'>
              {editorial.content}
            </p>
          </div>

          {/* Key Takeaways */}
          {editorial.keyTakeaways && editorial.keyTakeaways.length > 0 && (
            <div className='rounded-lg bg-muted/50 p-4'>
              <p className='mb-2 text-sm font-semibold'>Key Takeaways:</p>
              <ul className='space-y-1 text-sm text-muted-foreground'>
                {editorial.keyTakeaways.map((takeaway, index) => (
                  <li key={index} className='flex items-start gap-2'>
                    <span className='mt-1 text-primary'>•</span>
                    <span>{takeaway}</span>
                  </li>
                ))}
              </ul>
            </div>
          )}

          {/* Author Info */}
          <div className='flex items-center gap-2 border-t pt-2 text-xs text-muted-foreground'>
            <span>By {editorial.author}</span>
            <span>•</span>
            <time dateTime={editorial.timestamp}>
              {new Date(editorial.timestamp).toLocaleDateString('en-CA', {
                year: 'numeric',
                month: 'long',
                day: 'numeric',
              })}
            </time>
          </div>
        </CardContent>
      </Card>

      {/* Discussion Section */}
      {item.discussionEnabled && (
        <RSSItemComments itemId={id} itemTitle={item.title} />
      )}

      {/* Related News */}
      <div className='mt-12 rounded-lg bg-muted/50 p-8 text-center'>
        <h2 className='mb-4 text-2xl font-bold'>Want More Editorial Takes?</h2>
        <p className='mb-6 text-muted-foreground'>
          Get daily AI and tech news with editorialized takes and systems
          thinking analysis. Join the discussion on each news item.
        </p>
        <div className='flex flex-col justify-center gap-4 sm:flex-row'>
          <Button asChild size='lg'>
            <Link href='/rss-news'>View All News</Link>
          </Button>
          <Button asChild size='lg' variant='outline'>
            <Link href='/blog'>View Blog Articles</Link>
          </Button>
        </div>
      </div>
    </div>
  );
}

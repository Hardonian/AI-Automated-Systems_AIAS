      ...
      <div className='space-y-4'>
        {topLevelComments.map(comment => (
          <div key={comment.id} className='rounded-xl border p-3'>
            ...
            <div className='mb-2 text-sm'>{comment.body}</div>
            ...
            {replyingTo === comment.id && (
              <div className='mt-2 flex gap-2'>
                <input
                  ...
                  onKeyDown={e =>
                    e.key === 'Enter' && !e.shiftKey && addReply()
                  }
                />
                <button
                  className='h-8 rounded-lg bg-secondary px-3 text-sm'
                  onClick={addReply}
                >
                  Reply
                </button>
                <button
                  className='h-8 rounded-lg bg-muted px-3 text-sm'
                  onClick={() => {
                    setReplyingTo(null);
                    setReplyText('');
                  }}
            ...
            {repliesByParent[comment.id]?.map(reply => (
              <div
                key={reply.id}
                className='ml-6 mt-3 border-l-2 border-border pl-3'
              >

 ...
 */
export async function calculateActivationRate(
  days: number = 30
): Promise<number> {
  ...
  const signupCount = new Set(
    signups?.map((e: { user_id: string }) => e.user_id) || []
  ).size;
  const activationCount = new Set(
    activations?.map((e: { user_id: string }) => e.user_id) || []
  ).size;
 ...
 */
export async function calculateTimeToActivation(
  days: number = 30
): Promise<number> {
  const supabase = getSupabaseClient();
  if (!supabase) {
  ...
  const signupRows = (signups ?? []) as Array<{
    user_id: string;
    created_at: string;
  }>;
  const activationRows = (activations ?? []) as Array<{
    user_id: string;
    created_at: string;
  }>;

  // Create a Map for O(1) lookups instead of O(n) find() operations
  const signupMap = new Map(signupRows.map(s => [s.user_id, s.created_at]));
...
export async function calculateDay7Retention(): Promise<number> {
  ...
  const activeUserIds = new Set(
    activeUsers?.map((e: { user_id: string }) => e.user_id) || []
  );

  const retainedUsers = Array.from(signupUserIds).filter(id =>
    activeUserIds.has(id)
  );

  return signupUserIds.size > 0
    ? (retainedUsers.length / signupUserIds.size) * 100
    : 0;
}
...
export async function getFunnelMetrics(
  ...
  return {
    signups: new Set(
      signups.data?.map((e: { user_id: string }) => e.user_id) || []
    ).size,
    integrations: new Set(
      integrations.data?.map((e: { user_id: string }) => e.user_id) || []
    ).size,
    workflows: new Set(
      workflows.data?.map((e: { user_id: string }) => e.user_id) || []
    ).size,
    activations: new Set(
      activations.data?.map((e: { user_id: string }) => e.user_id) || []
    ).size,
  };
}
 ...
 */
export async function getAllActivationMetrics(
  days: number = 30
): Promise<ActivationMetrics> {
  const [activationRate, timeToActivation, day7Retention, funnel] =
    await Promise.all([

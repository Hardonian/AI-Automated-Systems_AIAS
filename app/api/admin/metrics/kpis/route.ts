...
export async function GET() {
  try {
    ...
    const { data: signupsThirtyDaysAgo } = await supabase
      ...
      ); // Signups from 30-37 days ago

    const currentRetention =
      signupsThirtyDaysAgo && signupsThirtyDaysAgo.length > 0
    ...
    const previousRetention =
      previousSignupsThirtyDaysAgo && previousSignupsThirtyDaysAgo.length > 0
        ? ((previousActiveUsers?.length || 0) /
            previousSignupsThirtyDaysAgo.length) *
          100
        : 0;

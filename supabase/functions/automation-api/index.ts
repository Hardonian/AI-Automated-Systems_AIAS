...
serve(async req => {
  ...
  try {
    ...
    switch (true) {
      ...
      case path.startsWith('/workflows/') &&
      ...
      case path.startsWith('/leads/') &&
        path.endsWith('/qualify') &&
        method === 'POST':
      ...
      case path.startsWith('/appointments/') &&
      ...
      case path.startsWith('/meetings/') &&
        path.endsWith('/notes') &&
        method === 'POST':
        return await handleCreateMeetingNotes(
          req,
          supabaseClient,
          user.id,
          path
      ...
      case path.startsWith('/design-projects/') &&
        path.endsWith('/versions') &&
        method === 'POST':
        return await handleCreateDesignVersion(
          req,
          supabaseClient,
          user.id,
          path
...
async function handleGetWorkflows(req: Request, supabase: any, userId: string) {
  ...
  const { data: workflows, error } = await query.order('updated_at', {
    ascending: false,
  });
...
async function handleCreateWorkflow(
  ...
  const { name, description, trigger, steps, category, tags }: WorkflowRequest =
    await req.json();
...
async function handleCreateLead(req: Request, supabase: any, userId: string) {
  const {
    ...
    source,
    qualificationData,
  }: LeadGenerationRequest = await req.json();
...
async function handleCreateAppointment(
  ...
  // Check for conflicts
  const { data: hasConflicts } = await supabase.rpc(
...
async function handleCreateMeetingNotes(
  req: Request,
  supabase: any,
  userId: string,
  path: string
...
// Design Projects
async function handleCreateDesignProject(
  req: Request,
  supabase: any,
  userId: string
) {
...
async function handleGetDesignProjects(
  req: Request,
  supabase: any,
  userId: string
) {
...
async function handleCreateDesignVersion(
  req: Request,
  supabase: any,
  userId: string,
  path: string
...
async function handleGetMetrics(req: Request, supabase: any, userId: string) {
  ...
  // Get various metrics
  const [workflowStats, executionStats, leadStats, appointmentStats] =
    await Promise.all([
      supabase
        .from('automation_workflows')
        .select('status')
  ...
  const metrics = {
    workflows: {
      ...
      active:
        workflowStats.data?.filter(w => w.status === 'active').length || 0,
    },
    executions: {
      total: executionStats.data?.length || 0,
      successful:
        executionStats.data?.filter(e => e.status === 'completed').length || 0,
      successRate: executionStats.data?.length
        ? (executionStats.data.filter(e => e.status === 'completed').length /
            executionStats.data.length) *
          100
        : 0,
    },
    leads: {
      ...
      qualified:
        leadStats.data?.filter(l => l.status === 'qualified').length || 0,
    },
    appointments: {
      total: appointmentStats.data?.length || 0,
      upcoming:
        appointmentStats.data?.filter(a => new Date(a.start_time) > new Date())
          .length || 0,
    },
  };

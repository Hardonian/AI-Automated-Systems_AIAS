...
export async function POST(
  ...
  try {
    const responseData: { received: boolean; processed?: boolean } = {
      received: true,
    };
    ...
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        if (!metadataValidation.success) {
          ...
          recordError(error, {
            endpoint: '/api/stripe/webhook',
            action: 'checkout_session_validation',
          });
          const formatted = formatError(error);
          return NextResponse.json(
        ...
        if (userId && tier) {
          ...
          try {
            ...
            logger.warn(
              'Failed to update Subscription table',
    ...
    try {
      ...
      logger.warn(
        'Failed to record idempotency for failed webhook',
        idempError instanceof Error ? idempError : new Error(String(idempError))
      );
    }

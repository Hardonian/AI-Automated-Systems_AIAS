...
async function main() {
  ...
  const workspaceMode = existsSync("pnpm-workspace.yaml")
    ? "workspace"
    : "single-package";
  console.log(
    `Toolchain: Node ${nodeVersion}, pnpm ${pnpmVersion}, mode=${workspaceMode}`,
  );
  ...
  if (!existsSync("next.config.mjs")) {
    categories.config.push("next.config.mjs missing");
  } else {
    try {
      await import(pathToFileURL(path.resolve("next.config.mjs")).href);
    } catch (err) {
      categories.config.push(`next.config.mjs failed to import: ${err}`);
    }
  }
  ...
  if (!existsSync("eslint.config.mjs")) {
    categories.config.push("eslint.config.mjs missing");
  } else {
    try {
      await import(pathToFileURL(path.resolve("eslint.config.mjs")).href);
    } catch (err) {
      categories.config.push(`eslint.config.mjs failed to import: ${err}`);
    }
  }
  ...
  if (existsSync(layoutPath)) {
    ...
    for (const match of layout.matchAll(/href=['\"](\/[^'\"]+)['\"]/g)) {
      ...
      if (!existsSync(asset))
        categories.assets.push(`missing public asset: ${match[1]}`);
    }
  }
  ...
  const failures = Object.entries(categories).filter(

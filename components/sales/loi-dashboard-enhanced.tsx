  ...
  const fetchLOIs = async () => {
    try {
      ...
      logger.error(
        ...
        {
          component: 'LOIDashboardEnhanced',
          action: 'fetchLOIs',
        }
      );
    } finally {
  ...
  return (
    <div className='space-y-6'>
      ...
      <motion.div
        ...
        {...({
          className:
            'relative overflow-hidden rounded-2xl bg-gradient-to-br from-blue-600 via-purple-600 to-pink-600 p-8 text-white shadow-2xl',
        } as any)}
      >
        ...
        >
          <Card className='border-2 border-blue-200 bg-gradient-to-br from-blue-50 to-white shadow-lg transition-all hover:shadow-xl'>
            ...
            <CardContent>
              ...
              <p className='mt-1 text-xs text-muted-foreground'>Target: 5</p>
          ...
          <Card className='border-2 border-green-200 bg-gradient-to-br from-green-50 to-white shadow-lg transition-all hover:shadow-xl'>
            ...
            <CardContent>
              ...
              <p className='mt-1 text-xs text-muted-foreground'>
                {summary.total > 0
                  ? ((summary.signed / summary.total) * 100).toFixed(0)
                  : 0}
                % signed
              </p>
            </CardContent>
          ...
          <Card className='border-2 border-purple-200 bg-gradient-to-br from-purple-50 to-white shadow-lg transition-all hover:shadow-xl'>
            <CardHeader className='pb-2'>
              <CardTitle className='flex items-center gap-2 text-sm font-medium'>
          ...
          <Card className='border-2 border-amber-200 bg-gradient-to-br from-amber-50 to-white shadow-lg transition-all hover:shadow-xl'>
            <CardHeader className='pb-2'>
              <CardTitle className='flex items-center gap-2 text-sm font-medium'>
              ...
              <div className='space-y-4'>
                {lois.map((loi, loiIndex) => (
                  <motion.div
                    ...
                    {...({
                      className:
                        'border-2 rounded-xl p-6 space-y-4 bg-gradient-to-br from-white to-gray-50 hover:shadow-lg transition-all',
                    } as any)}
                  >
                    <div className='flex items-center justify-between'>
                      <div>
                        <h3 className='text-xl font-bold'>{loi.company}</h3>
                        <p className='text-sm text-muted-foreground'>
                          {loi.contact} • {loi.industry} • {loi.companySize}{' '}
                          employees
                        </p>
                      </div>
                    ...
                    <div className='grid grid-cols-2 gap-4 md:grid-cols-4'>
                      <div className='rounded-lg bg-blue-50 p-3'>
                        ...
                        <p className='font-bold text-blue-600'>
                          ${loi.monthlyCommitment.toLocaleString()}
                        </p>
                      </div>
                      <div className='rounded-lg bg-purple-50 p-3'>
                        ...
                        <p className='font-bold text-purple-600'>
                          ${loi.annualValue.toLocaleString()}
                        </p>
                      </div>
                    ...
                    {loi.dateSigned && (
                      <div className='flex items-center gap-2 rounded-lg bg-green-50 p-3 text-sm text-green-600'>
                        ...
                        <span>
                          Signed:{' '}
                          {new Date(loi.dateSigned).toLocaleDateString()}
                          {loi.validUntil &&
                            ` • Valid until: ${new Date(loi.validUntil).toLocaleDateString()}`}
                        </span>
                      </div>
                    )}
                  </motion.div>
                  ...
                  <ResponsiveContainer height={300} width='100%'>
                    <BarChart
                      data={[
                        ...
                        {
                          tier: 'Enterprise',
                          count: summary.byTier.Enterprise,
                        },
                      ]}
                    >

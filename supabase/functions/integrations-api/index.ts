    ...
    switch (method) {
      ...
      case 'PUT':
        if (integrationId && action === 'config') {
          ...
          const body = await req.json();
          return await updateIntegrationConfig(
            supabaseClient,
            integrationId,
            body,
            tenantId,
...
async function getIntegrations(
  ...
  const { data: availableIntegrations, error: availableError } =
  ...
  if (availableError) {
    return new Response(JSON.stringify({ error: availableError.message }), {
      status: 400,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  }
  ...
  const { data: connectedIntegrations, error: connectedError } =
    await supabaseClient
  ...
  if (connectedError) {
    return new Response(JSON.stringify({ error: connectedError.message }), {
      status: 400,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  }
...
async function connectIntegration(
  ...
  const validationResult = await validateIntegrationConfig(
    integration,
    body.configuration
  );
...
// Helper functions
async function validateIntegrationConfig(
  integration: any,
  config: any
...
async function testSlackConnection(config: any, credentials: any) {
  ...
  return {
    ...
    data: { workspaceId: 'test_workspace_123' },
  };
}
...
async function testGoogleConnection(config: any, credentials: any) {
  ...
  return {
    ...
    data: { accountId: 'test_account_123' },
  };
}
...
async function testZapierConnection(config: any, credentials: any) {
  ...
  return {
    ...
    data: { accountId: 'test_zapier_123' },
  };
}

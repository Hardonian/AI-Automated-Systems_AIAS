...
export function ROICalculator() {
  ...
  const calculations = useMemo(() => {
    ...
    const roi =
      annualValue > 0 ? ((annualValue - annualCost) / annualCost) * 100 : 0;
  ...
  return (
    <Card className='mt-12'>
      <CardHeader>
        ...
        <CardDescription>
          Calculate how much time and money you'll save with AIAS Platform
          automation.
        </CardDescription>
      </CardHeader>
            ...
            <div className='grid grid-cols-2 gap-4'>
              <div>
                ...
                <p className='text-2xl font-bold'>
                  {monthlyHoursSaved.toFixed(1)} hours
                </p>
              </div>
              <div>
            ...
            {roi > 0 && (
              <div className='mt-4 rounded-lg border border-green-200 bg-green-50 p-4 dark:border-green-800 dark:bg-green-950/20'>
                <p className='text-sm text-green-900 dark:text-green-100'>
                  <strong>Great news!</strong> You'll save $
                  {netAnnualSavings.toFixed(2)} per year with a {roi.toFixed(0)}
                  % ROI. That's like getting paid $
                  {(netAnnualSavings / 12).toFixed(2)}

...
class LoadTester {
  ...
  async run(): Promise<TestReport> {
    ...
    const rampUpInterval =
      (this.config.rampUpTime * 1000) / this.config.targetUsers;
  ...
  private generateReport(): TestReport {
    ...
    const failedRequests = this.metrics.filter(
      m => m.statusCode < 200 || m.statusCode >= 300
    );
    ...
    for (const endpoint of this.config.endpoints) {
      const endpointMetrics = this.metrics.filter(
        m => m.endpoint === endpoint.path
      );
      const endpointResponseTimes = endpointMetrics
        .map(m => m.responseTime)
        .sort((a, b) => a - b);

      endpointStats[endpoint.path] = {
        requests: endpointMetrics.length,
        success: endpointMetrics.filter(
          m => m.statusCode >= 200 && m.statusCode < 300
        ).length,
        failures: endpointMetrics.filter(
          m => m.statusCode < 200 || m.statusCode >= 300
        ).length,
        avgResponseTime:
          endpointResponseTimes.reduce((a, b) => a + b, 0) /
            endpointResponseTimes.length || 0,
    ...
    for (let t = this.startTime; t < Date.now(); t += interval) {
      ...
      timeline.push({
        ...
        avgResponseTime:
          windowResponseTimes.reduce((a, b) => a + b, 0) /
            windowResponseTimes.length || 0,
        errorRate:
          windowMetrics.filter(m => m.statusCode < 200 || m.statusCode >= 300)
            .length / windowMetrics.length || 0,
      });
    }
    ...
    const costPerUser =
      (estimatedCostPerHour / 3600) * (totalDuration / this.config.targetUsers);

    const recommendations: string[] = [];
    if (avgResponseTime > 1000) {
      recommendations.push(
        'Average response time exceeds 1s - consider optimizing slow endpoints'
      );
    }
    if (p95 > 2000) {
      recommendations.push(
        '95th percentile latency exceeds 2s - investigate performance bottlenecks'
      );
    }
    ...
    if (avgResponseTime < 200 && p95 < 500) {
      recommendations.push(
        'Performance is excellent - system can handle higher load'
      );
    }
...
async function main() {
  ...
  for (let i = 0; i < args.length; i += 2) {
    ...
    if (key === 'users') {
      config.targetUsers = parseInt(value || '1000');
    }
    if (key === 'duration') {
      config.duration = parseInt(value || '300');
    }
    if (key === 'ramp-up') {
      config.rampUpTime = parseInt(value || '60');
    }
    if (key === 'base-url') {
      config.baseUrl = value || 'http://localhost:3000';
    }
  }
  ...
  console.log(
    `   Avg Response Time: ${report.summary.averageResponseTime.toFixed(2)}ms`
  );
  ...
  console.log(
    `   Requests/sec: ${report.summary.requestsPerSecond.toFixed(2)}`
  );

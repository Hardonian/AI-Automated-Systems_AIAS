...
function createBuildSafeHandler(
  ...
  createBuildSafeHandler(async (context: RouteContext) => {
    ...
    try {
      ...
      const activationRate =
        totalSignups > 0 ? (totalActivations / totalSignups) * 100 : 0;
      ...
      if (signups && activations) {
        for (const activation of activations) {
          const signup = signups.find(
            (s: { user_id: string }) => s.user_id === activation.user_id
          );
          if (signup) {
      ...
      const medianTimeToActivation =
        activationTimes.length > 0
          ? activationTimes.sort((a, b) => a - b)[
              Math.floor(activationTimes.length / 2)
            ]
          : 0;
      ...
      const signupUserIds = new Set(
        signups7DaysAgo?.map((e: { user_id: string }) => e.user_id) || []
      );
      ...
      return NextResponse.json({
        metrics: {
          ...
          time_to_activation_hours:
            Math.round(
              ((medianTimeToActivation ?? 0) / (1000 * 60 * 60)) * 100
            ) / 100,
